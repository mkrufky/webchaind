...
// webchaind is the official command-line client for Webchain.
package main
...
var makeMlogDocCommand = cli.Command{
	...
	so 'webchaind mdoc -md discover' will generate markdown documentation only
...
func makeCLIApp() (app *cli.App) {
	...
	app.Version = Version
	app.Usage = "the webchaind command line interface"
	app.Action = geth
	...
	app.CommandNotFound = func(c *cli.Context, command string) {
		fmt.Fprintf(c.App.Writer, "Invalid command: %q. Please find `webchaind` usage below. \n", command)
		cli.ShowAppHelp(c)
		os.Exit(3)
	}
	return app
