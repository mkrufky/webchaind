...
import (
	...
	"github.com/mkrufky/webchaind/miner"
...
func makeNodeName(version string, ctx *cli.Context) string {
	name := fmt.Sprintf("webchaind/%s/%s/%s", version, runtime.GOOS, runtime.Version())
...
func MakeSystemNode(version string, ctx *cli.Context) *node.Node {
	...
	if ctx.GlobalBool(Unused1.Name) {
		glog.V(logger.Warn).Warnln(fmt.Sprintf("webchaind started with --%s flag, which is unused and can be omitted", Unused1.Name))
	}
...
func mustMakeSufficientChainConfig(ctx *cli.Context) *core.SufficientChainConfig {
	...
	if _, de := os.Stat(defaultChainConfigPath); de != nil && os.IsNotExist(de) {
		...
		$ webchaind --chain morden dump-chain-config %v/chain.json
...
func logChainConfiguration(ctx *cli.Context, config *core.SufficientChainConfig) {
	...
	glog.V(logger.Info).Infof("Starting webchaind %s", ctx.App.Version)
	glog.D(logger.Warn).Infof("webchaind version: %s", logger.ColorGreen(ctx.App.Version))

	glog.V(logger.Info).Infof("webchaind is configured to use blockchain: %v", config.Name)
