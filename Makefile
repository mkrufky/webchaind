...
build: cmd/abigen cmd/bootnode cmd/disasm cmd/ethtest cmd/evm cmd/gethrpctest cmd/rlpdump cmd/webchaind ## Build a local snapshot binary version of all commands
	@ls -ld $(BINARY)/*

cmd/webchaind: chainconfig ## Build a local snapshot binary version of geth.
	mkdir -p ./${BINARY}
	${GO_MOD} go build ${LDFLAGS} -o ${BINARY}/webchaind -tags="netgo" ./cmd/webchaind
...
cmd/ethtest: ## Build a local snapshot of ethtest.
	mkdir -p ./${BINARY} && CGO_CFLAGS_ALLOW='-maes.*' ${GO_MOD} go build ${LDFLAGS} -o ${BINARY}/ethtest ./cmd/ethtest
...
install_webchaind: chainconfig ## Install geth to $GOPATH/bin
	$(info Installing $$GOPATH/bin/webchaind)
	CGO_CFLAGS_ALLOW='-maes.*' ${GO_MOD} go install ${LDFLAGS} -tags="netgo" ./cmd/webchaind
...
clean: ## Remove local snapshot binary directory
	...
	go clean -i ./...
	rm -rf vendor/github.com/mkrufky/sputnikvm-ffi/c/ffi/target
	rm -f vendor/github.com/mkrufky/sputnikvm-ffi/c/libsputnikvm.*
